...
function stv2_propstack_api_key(): string
{
    $config = inside_config();
    return trim((string) (($config['integrations'] ?? [])['propstack_api_key'] ?? ''));
}

function stv2_propstack_request(string $path, array $query = [], string $version = 'v2'): array
{
    $apiKey = stv2_propstack_api_key();
    if ($apiKey === '') {
...
function stv2_propstack_items(array $response): array
{
...
function stv2_propstack_fetch_properties(int $maxPages = 12, int $perPage = 50): array
{
    $properties = [];
    for ($page = 1; $page <= $maxPages; $page++) {
        $response = stv2_propstack_request('/properties', [
            'per' => $perPage,
            'page' => $page,
            'sort_by' => 'updated_at',
            'order' => 'desc',
        ]);
        $items = stv2_propstack_items($response);
...
function stv2_propstack_fetch_unit_supplement(int $id): array
{
    try {
        return stv2_propstack_request('/units/' . $id, [], 'v1');
    } catch (Throwable) {
        return [];
    }
}
...
try {
    $properties = stv2_propstack_fetch_properties(12, 50);
    ...
    foreach ($properties as $property) {
        ...
        $unit = stv2_propstack_fetch_unit_supplement($id);
