...
export const immobilienbewertungTemplate: LandingTemplate = {
  ...
  h1: (location) => `Immobilienbewertung ${formatLocationPhraseFromName(location)}`,
  title: (location) => `Immobilienbewertung ${formatLocationPhraseFromName(location)} – realistische Preiseinschätzung`,
  description: (location) =>
    `Was ist deine Immobilie ${formatLocationPhraseFromName(location)} wert? Jetzt fundierte Bewertung erhalten.`,
  intro: (location) =>
    `Was ist deine Immobilie ${formatLocationPhraseFromName(location)} aktuell wirklich wert? Erhalte eine realistische Preisspanne – basierend auf echten Verkaufsdaten aus ${location}.`,
  text1: () =>
  ...
  faq: (location) => [
    {
      question: `Was kostet eine Immobilienbewertung ${formatLocationPhraseFromName(location)}?`,
      answer:
        `Die erste Einschätzung deiner Immobilie ${formatLocationPhraseFromName(location)} ist unverbindlich. Ziel ist eine realistische Orientierung, bevor du über weitere Schritte entscheidest.`,
    },
    {
    ...
    },
    {
      ...
      answer:
        `Der durchschnittliche Quadratmeterpreis ist nur ein Orientierungswert. Zwei Immobilien ${formatLocationPhraseFromName(location)} können trotz ähnlicher Größe deutlich unterschiedliche Marktwerte haben.`,
    },
    {
