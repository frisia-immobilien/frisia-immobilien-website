...
export default function LocationHero({ data }: { data: LocationPageData }) {
  return (
    <>
      <MobileHeroSection
        eyebrow={`${data.template.label} in Ostfriesland`}
        title={data.content.h1}
        description={data.content.intro}
        imageSrc={data.image.src}
        imageAlt={data.image.alt}
        imagePosition="58% center"
        primaryCta={{ href: "/immobilie-bewerten", label: "Bewertung starten" }}
        secondaryCta={{ href: PHONE_HREF, label: "Einfach kurz sprechen", sublabel: PHONE_DISPLAY }}
        trustItems={["Regional", "Marktdaten", "Persönlich"]}
      />

      <section className="mx-auto hidden w-full max-w-[1240px] gap-10 px-4 py-12 sm:px-6 md:grid md:grid-cols-12 md:items-center md:py-16">
        <div className="md:col-span-7">
          ...
          <h1 className="mt-3 max-w-full break-normal font-[family-name:var(--font-playfair)] text-[clamp(2.35rem,4.6vw,4.4rem)] leading-[1.04] text-[color:var(--color-navy)] [hyphens:none] [overflow-wrap:normal]">
            {data.content.h1}
          </h1>
          <HeroDivider />
